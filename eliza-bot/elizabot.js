...
ElizaBot.prototype.transform = function(text) {
	...
	// 调试信息：记录原始输入
	console.log("🔍 ELIZA 调试开始");
	console.log("📝 原始输入:", text);
	debugger; // 第一个断点：查看原始输入
	
	// unify text string
	text=text.toLowerCase();
	console.log("🔄 转换为小写:", text);
	...
	for (var i=0; i<parts.length; i++) {
		...
		if (part!='') {
			...
			for (var k=0; k<this.elizaKeywords.length; k++) {
				...
				var rank = this.elizaKeywords[k][1];
				console.log(`  ${k+1}. "${keyword}" (优先级: ${rank})`);
			}
			
			debugger; // 第二个断点：查看关键词匹配过程
			
			for (var k=0; k<this.elizaKeywords.length; k++) {
				var keyword = this.elizaKeywords[k][0];
				var rank = this.elizaKeywords[k][1];
				
				// 对于中文关键词，不使用单词边界；对于英文关键词，使用单词边界
				var regex;
				if (/[\u4e00-\u9fa5]/.test(keyword)) {
					// 中文关键词，直接匹配
				...
				var found = part.search(regex)>=0;
				
				console.log(`🔎 检查关键词 "${keyword}" (优先级: ${rank}): ${found ? '✅ 匹配!' : '❌ 不匹配'}`);
				
				if (found) {
					console.log(`🎯 找到匹配的关键词: "${keyword}"`);
					console.log(`⭐ 优先级: ${rank}`);
					debugger; // 第三个断点：找到匹配时
					rpl = this._execRule(k);
					console.log(`💬 生成的回复: "${rpl}"`);
				}
				if (rpl!='') {
	...
	console.log("❌ 没有找到匹配的关键词，尝试记忆库");
	rpl=this._memGet();
	// if nothing in mem, so try xnone
	if (rpl=='') {
		console.log("💭 记忆库为空，使用默认回复 (xnone)");
		this.sentence=' ';
		var k=this._getRuleIndexByKey('xnone');
		if (k>=0) rpl=this._execRule(k);
	}
	// return reply or default string
	console.log("🔚 ELIZA 调试结束");
...
ElizaBot.prototype._execRule = function(k) {
	...
	console.log(`\n🎯 执行规则: "${keyword}" (优先级: ${rank})`);
	console.log(`📝 当前句子: "${this.sentence}"`);
	console.log(`🔍 该关键词有 ${decomps.length} 个模式规则:`);
	
	for (var i=0; i<decomps.length; i++) {
		console.log(`  模式 ${i+1}: ${decomps[i][0]}`);
		console.log(`  回复模板: [${decomps[i][1].join(', ')}]`);
	}
	
	debugger; // 第四个断点：查看规则执行过程
	
	for (var i=0; i<decomps.length; i++) {
		var pattern = decomps[i][0];
		var m=this.sentence.match(pattern);
		console.log(`🔎 测试模式 ${i+1}: "${pattern}" -> ${m ? '✅ 匹配' : '❌ 不匹配'}`);
		
		if (m!=null) {
			console.log(`🎉 模式匹配成功!`);
			if (m.length > 1) {
				console.log(`📋 捕获的参数:`, m.slice(1));
			}
			...
			console.log(`🎲 可用回复模板 ${reasmbs.length} 个:`);
			reasmbs.forEach((template, idx) => {
				console.log(`  ${idx}: "${template}"`);
			});
			console.log(`🎯 选中模板 ${ri}: "${reasmbs[ri]}"`);
			...
			console.log(`💬 最终选择的回复模板: "${rpl}"`);
			debugger; // 第五个断点：查看回复生成过程
			...
			if (rpl.search('^goto ', 'i')==0) {
				console.log(`🔄 跳转到其他规则: ${rpl.substring(5)}`);
			...
			if (m1) {
				...
				rpl=lp+rp;
				console.log(`✅ 参数替换完成: "${rpl}"`);
			}
			rpl=this._postTransform(rpl);
			console.log(`🎨 最终回复: "${rpl}"`);
